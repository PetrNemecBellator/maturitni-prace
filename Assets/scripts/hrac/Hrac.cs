...
public class Hrac : MonoBehaviour
{
  ...
  /*  public void reverseMoveUnitsInGroup(int groupNumber, Vector2 vectorOfmovementReverseTOPreviusOne, Vector2 componentOfWar)
    ...
    public void changeLocationOfPlayersGroup(int groupNumber, Vector2 newVector)
    {
        for (int x =0; x < this.groupsOfUnits[groupNumber].Count; x++)
        {
            mapCreation.changeLocationOfUnit(this.groupsOfUnits[groupNumber][x].gameObject, newVector);
        }

       // CheckFight();
       //update type of unit currently having
